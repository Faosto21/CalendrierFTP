...
menu_personne = ttk.Combobox(
    barre, textvariable=personne_var, values=noms, state="readonly", width=12
)
...
def gestion_semaine():
    ...
    btn_precedent.config(
        state=("disabled" if semaine_courante <= SEMAINE_MIN else "normal")
    )
    btn_suivant.config(
        state=("disabled" if semaine_courante >= SEMAINE_MAX else "normal")
    )
